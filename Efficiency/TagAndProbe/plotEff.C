...
void plotEff(const TString conf,            // input binning file
  ...
  const Double_t massLo    = 66;
  const Double_t massHi    = 116;
...
// for nominal iso cone, 0.15
  if(yaxislabel.CompareTo("Sta")==0       && bkgfail==6 && charge==0 && xbinLo==0.0 && xbinHi==0.9){
    bkgFail = new CQuadratic(m,kFALSE,ibin,-14.178,14.3019 ,0.531948,0.335449,-0.00304265,0.00186565);
    nflfail += 3;
  }else if(yaxislabel.CompareTo("Sta")==0       && bkgfail==6 && charge==0 && xbinLo==0.9 && xbinHi==2.4){
    bkgFail = new CQuadratic(m,kFALSE,ibin,-6.62591,14.155 ,0.357071,0.329991,-0.00215787,0.00182641);
    nflfail += 3;
/*  if(yaxislabel.CompareTo("Sta")==0       && bkgfail==6 && charge==0 && xbinLo==0.0 && xbinHi==0.9){
    bkgFail = new CQuadratic(m,kFALSE,ibin,-40.099,27.9344,1.77845,0.649996,-0.0106337,0.00359035);
    nflfail += 3;*/
  }else if(yaxislabel.CompareTo("Sta")==0       && bkgfail==6 && charge==0 && xbinLo==0.9 && xbinHi==2.1){
    bkgFail = new CQuadratic(m,kFALSE,ibin,11.3889,25.918,0.473894,0.601494,-0.00342159,0.00331999);
    nflfail += 3;
  }else if(yaxislabel.CompareTo("Sta")==0       && bkgfail==6 && charge==0 && xbinLo==0.0 && xbinHi==2.4){
    bkgFail = new CQuadratic(m,kFALSE,ibin,-24.851,14.3007 ,0.788767,0.336036,-0.00452871,0.00186935);
    nflfail += 3;
  ...
  } else if(yaxislabel.CompareTo("Sta")==0       && bkgfail==6 && charge==0 && xbinLo== 0.9 && xbinHi== 2.4 && ybinLo==30 && ybinHi==8000){
    bkgFail = new CQuadratic(m,kFALSE,ibin,46.7726,15.6278,-0.859772,0.36628,0.00468633,0.00204896);
    nflfail += 3;
...
// for 76X 
//  if(yaxislabel.CompareTo("SIT")==0       && charge==0 && xbinLo== 0.0 && xbinHi== 0.9 && ybinLo==27 && ybinHi==8000) { eff.setVal(0.80);eff.setRange(0,0.90);Nsig.setRange(0,5.0*NsigMax);}
...
// for 76X
  ...
  if(yaxislabel.CompareTo("GsfSel")==0    && charge==0 && xbinLo==-0.5    && xbinHi== 0.0    && ybinLo==25 && ybinHi==  40) strategy = 1;
  ...
  if(yaxislabel.CompareTo("GsfSel")==0    && charge==0 && xbinLo==-2.5    && xbinHi==-2.0    && ybinLo==25 && ybinHi==  40) strategy = 1;
  if(yaxislabel.CompareTo("GsfSel")==0    && charge==0 && xbinLo== 1.0    && xbinHi== 1.4442 && ybinLo==25 && ybinHi==  40) strategy = 1;
  if(yaxislabel.CompareTo("GsfSel")==0    && charge==0 && xbinLo== 1.566  && xbinHi== 2.0    && ybinLo==25 && ybinHi==  40) strategy = 1;
  if(yaxislabel.CompareTo("GsfSel")==0    && charge==0 && xbinLo== 2.0    && xbinHi== 2.5    && ybinLo==25 && ybinHi==  40) strategy = 1;
  if(yaxislabel.CompareTo("GsfSel")==0    && charge==0 && xbinLo==-2.0    && xbinHi==-1.566  && ybinLo==55 && ybinHi==8000) strategy = 1;
  if(yaxislabel.CompareTo("GsfSel")==0    && charge==0 && xbinLo== 1.566  && xbinHi== 2.0    && ybinLo==55 && ybinHi==8000) strategy = 1;
